...
func buildPostDeployDriftFinding(app model.LaravelApp, snapshot model.Snapshot) (model.Finding, bool) {
	...
	return model.Finding{
		...
		Title:       "Post-deploy drift weakens Laravel ownership or writable-path boundaries",
		Why:         "The deployed Laravel tree no longer matches the intended ownership, writable-path, symlink, or release-boundary shape. Without an explicit post-deploy verification step, routine rollout changes can quietly leave code, .env, or shared paths in a weaker state than operators expect.",
		Remediation: "Add a post-deploy verification step that confirms code and .env stay non-runtime-writable, writable paths stay limited to storage/ and bootstrap/cache/, symlink targets stay intentional, and previous releases remain immutable.",
...
func buildPostRestoreDriftFinding(app model.LaravelApp, snapshot model.Snapshot) (model.Finding, bool) {
	...
	return model.Finding{
		...
		Title:       "Post-restore drift weakens Laravel ownership, shared-path, or recovery boundaries",
		Why:         "Restore or backup workflows are present for this app and the current snapshot shows drift across code ownership, runtime write boundaries, symlink targets, releases, or recovery artifact placement. Recovery steps need a separate verification pass because incident pressure makes stale permissions and shared-path mistakes much easier to miss.",
		Remediation: "After each restore, re-verify code and .env ownership, runtime write access, shared-path symlinks, release immutability, and backup artifact placement before putting the app back in service.",
		Evidence:    correlation.evidence,
		Affected:    correlation.affected,
	}, true
}
